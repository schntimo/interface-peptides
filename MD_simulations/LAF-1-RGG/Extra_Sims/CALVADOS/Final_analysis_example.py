...
# %% Inputs (plotting limits might need to be adjusted)

prot_name = 'prot'
xml_file =  './seed1/system.xml'
pdb_file =  './seed1/top.pdb'
path_traj_files = ['./seed1/traj.dcd',
                   './seed2/traj.dcd',
                   './seed3/traj.dcd',
                   './seed4/traj.dcd',
                   './seed5/traj.dcd',
...
first_step_eval = 100000   # timestep 
stride = 1
n_prot_mol = 16
...
for i_sim, path_traj in enumerate(path_traj_files):
    ...
    u = mda.Universe(pdb_file, path_traj, format='DCD') 
    u.add_TopologyAttr('masses', masses)
      
    #define atom groups
    prot_all = u.segments[:n_prot_mol].atoms
    pep_all = u.segments[n_prot_mol:].atoms
...
plt.plot(centers_time/1e3, binned_mean_com, label="Mean", color="blue")
plt.fill_between(centers_time/1e3, binned_mean_com - binned_std_com/len(path_traj_files)**0.5, binned_mean_com + binned_std_com/len(path_traj_files)**0.5, alpha=0.3, color="blue")
plt.xlabel("Time [ns]")
...
plt.figure(figsize=(4.5, 2.8))
plt.plot(centers_time/1e3, cd_mean, color="red")
plt.fill_between(centers_time/1e3, cd_mean - cd_std/len(path_traj_files)**0.5, cd_mean + cd_std/len(path_traj_files)**0.5, alpha=0.3, color="red", label='Dense')
plt.xlabel("Time [ns]")

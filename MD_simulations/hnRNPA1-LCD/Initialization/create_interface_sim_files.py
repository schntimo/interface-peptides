...
path_template = './Simulation_templates/Interface'
path_sequence_files = './initial_sequences_nodelim_V2.txt'
path_folder_sims = './Simulation_files_interface'
...
for i, seq in enumerate(sequences):
    ...
    for root, dirs, files in os.walk(folder_sim):
        for filename in fnmatch.filter(files, 'Fr_*'):
            file_path = os.path.join(root, filename)
            with open(file_path, 'r') as file:
                lines = file.readlines()
        
            new_lines = []
            for ind, line in enumerate(lines[2296:2356]):
                parts = line.split()
                parts[2] = str(AA_numbers[ind])
                new_lines.append(' '.join(parts) + '\n')
            lines[2296:2356] = new_lines
            with open(file_path, 'w') as file:
                file.writelines(lines)
